...
3.如果从前往后复制每个数字（或字符）需要移动数字（或字符）多次，可以考虑从后往前复制。<br/>
4.把一个整数减去1之后再和原来的整数做位与运算，得到的结果相当于是把整数的二进制表示中的最右边一个1变成0.<br/>
5.当用一个指针遍历链表不能解决问题的时候，可以使用两个指针遍历链表。可以让其中一个指针遍历的速度快一点，或者让它先在链表上走若干步。<br/>
